...
function catIcon(icon) {
  return CAT_ICON_MAP[icon] ?? 'fa-solid fa-puzzle-piece';
}

const detectedEntries = computed(() => (data.value ? Object.entries(data.value.detected) : []));
...
async function analyze() {
  ...
  loading.value = true;
  try {
    const res = await fetch('/api/tech-stack-detector', {
      method: 'POST',
...
<template>
    ...
    <div class="relative z-10">
      <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-7">
        <div class="min-w-0">
          ...
          <h2 class="font-sans text-2xl sm:text-3xl font-700 tracking-tight leading-tight">Enter a URL to detect tech.</h2>
  ...
  <div v-if="data" class="space-y-4">
    <div class="grid grid-cols-2 gap-3">
      <StatTile :value="data.total" label="Technologies" color="#a3e635" />
      <StatTile :value="`${data.loadTime}ms`" label="Load Time" color="#0694a2" />
    </div>

    <div v-if="detectedEntries.length === 0" class="glass rounded-3xl p-8 text-center">
      <i class="fa-solid fa-circle-question text-3xl mb-3" style="color:#f97316"></i>
      <p class="text-sm font-semibold">No known technologies detected.</p>
    </div>
    <div v-else class="grid sm:grid-cols-2 gap-4">
      <div v-for="[cat, info] in detectedEntries" :key="cat" class="glass rounded-3xl p-5">
        <div class="flex items-center gap-3 mb-3">
          <div class="w-10 h-10 rounded-xl flex items-center justify-center" style="background:linear-gradient(135deg,#0694a2,#a3e635)">
            <i :class="catIcon(info.icon)" class="text-sm" style="color:#061c21"></i>
          </div>
          <p class="text-sm font-700">{{ cat }}</p>
        </div>
        <div class="flex flex-wrap gap-2">
          <span v-for="t in info.techs" :key="t" class="text-xs font-semibold px-3 py-1.5 rounded-full" style="background:rgba(163,230,53,0.10);border:1px solid rgba(163,230,53,0.25);color:var(--c-text)">{{ t }}</span>
        </div>
      </div>
    </div>
  </div>
