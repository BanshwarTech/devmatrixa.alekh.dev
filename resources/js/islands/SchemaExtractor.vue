...
async function analyze() {
  ...
  loading.value = true;
  try {
    const res = await fetch('/api/schema-extractor', {
      method: 'POST',
...
<template>
    ...
    <div class="relative z-10">
      <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-7">
        <div class="min-w-0">
          ...
          <h2 class="font-sans text-2xl sm:text-3xl font-700 tracking-tight leading-tight">Enter a URL to extract JSON-LD.</h2>
  ...
  <div v-if="data" class="space-y-6">
    <div v-if="data.total === 0" class="glass rounded-3xl p-8 text-center">
      <i class="fa-solid fa-triangle-exclamation text-3xl mb-3" style="color:#f97316"></i>
      <p class="text-sm font-semibold">No JSON-LD structured data found.</p>
      <p class="text-xs mt-1" style="color:var(--c-muted)">Add &lt;script type="application/ld+json"&gt; tags relevant to your page.</p>
    </div>

    <template v-else>
      <div class="glass rounded-3xl p-5">
        <div class="flex items-center justify-between gap-3 mb-3 flex-wrap">
          <p class="text-xs font-bold uppercase tracking-widest" style="color:var(--c-muted)">{{ data.total }} Schemas Found</p>
          <div class="flex flex-wrap gap-2">
            <span v-for="(n, t) in data.typeCounts" :key="t" class="text-xs px-3 py-1 rounded-full" style="background:rgba(163,230,53,0.12);color:#65a30d;border:1px solid rgba(163,230,53,0.25)">
              {{ t }} <strong>&times;{{ n }}</strong>
            </span>
          </div>
        </div>
      </div>

      <div v-for="(s, i) in data.schemas" :key="i" class="glass rounded-3xl overflow-hidden">
        <div class="px-5 py-3 flex items-center justify-between" style="border-bottom:1px solid var(--c-border);background:rgba(255,255,255,0.06)">
          <span class="text-xs font-bold uppercase tracking-widest" style="color:#0694a2">{{ s.type }}</span>
          <button @click="copy(s.json, i)" class="btn-outline text-xs px-3 py-1 rounded-full inline-flex items-center gap-1.5">
            <i :class="copied === i ? 'fa-solid fa-check' : 'fa-solid fa-copy'" class="text-[10px]"></i>
            {{ copied === i ? 'Copied!' : 'Copy JSON' }}
          </button>
        </div>
        <pre class="p-4 text-[11px] overflow-x-auto font-mono leading-relaxed" style="background:#0a2f36;color:#daf2f4"><code>{{ s.json }}</code></pre>
      </div>
    </template>
  </div>
</template>
