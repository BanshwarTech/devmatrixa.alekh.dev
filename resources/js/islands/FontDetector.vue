...
const sourceBg = {
  'Google Fonts': { bg: 'rgba(101,163,13,0.12)', col: '#65a30d' },
  'Custom (@font-face)': { bg: 'rgba(6,148,162,0.12)', col: '#0694a2' },
  'CSS Stack': { bg: 'rgba(22,189,202,0.12)', col: '#16bdca' },
  'System Font': { bg: 'rgba(126,220,226,0.12)', col: '#7edce2' },
};

function sourceStyle(source) {
  return sourceBg[source] ?? sourceBg['CSS Stack'];
}

function fontFamilyFor(f) {
  return f.source === 'Google Fonts' ? `'${f.family}', sans-serif` : f.family;
}
...
async function analyze() {
  ...
  loading.value = true;
  try {
    const res = await fetch('/api/font-detector', {
      method: 'POST',
...
<template>
    ...
    <div class="relative z-10">
      <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-7">
        <div class="min-w-0">
          ...
          <h2 class="font-sans text-2xl sm:text-3xl font-700 tracking-tight leading-tight">Enter a URL to detect fonts.</h2>
  ...
  <div v-if="data" class="space-y-4">
    <div class="grid grid-cols-2 gap-3">
      <StatTile :value="data.total" label="Fonts" color="#a3e635" />
      <StatTile :value="`${data.loadTime}ms`" label="Load Time" color="#0694a2" />
    </div>

    <div class="space-y-3">
      <div v-for="(f, i) in data.fonts" :key="i" class="glass rounded-2xl p-5">
        <div class="flex flex-wrap items-center justify-between gap-3 mb-2">
          <p class="text-xl font-700" :style="{ fontFamily: fontFamilyFor(f), color: 'var(--c-text)' }">{{ f.family }}</p>
          <span class="text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-full" :style="{ background: sourceStyle(f.source).bg, color: sourceStyle(f.source).col }">{{ f.source }}</span>
        </div>
        <p class="text-2xl mb-3" :style="{ fontFamily: fontFamilyFor(f), color: 'var(--c-muted)' }">The quick brown fox jumps over the lazy dog</p>
        <div class="flex flex-wrap gap-2 text-xs" style="color:var(--c-muted)">
          <span v-if="f.weights.length > 0">Weights: <strong style="color:var(--c-text)">{{ f.weights.join(', ') }}</strong></span>
          <span v-if="f.usageCount > 0">· Used <strong style="color:#a3e635">{{ f.usageCount }}×</strong></span>
        </div>
      </div>
    </div>
  </div>
